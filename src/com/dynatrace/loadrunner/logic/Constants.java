...
public class Constants {
	public final static String ADD_HEADER_FUNCTION_TEST="addDynatraceHeaderTest";

	public static final String CRLF_OS_INDEPENDENT = "\r\n";
	
	public final static String DT_FUNCTION_TEST = CRLF_OS_INDEPENDENT + "void addDynatraceHeaderTest(char* header){" + CRLF_OS_INDEPENDENT
			...
			+ "	char* vuserstring=(char*) malloc(sizeof(char) * 10);" + CRLF_OS_INDEPENDENT
			...
			+ "	headerValueLength = strlen(header) + 4 + strlen(vuserstring) + 4 + strlen(ltnString) + 4;" + CRLF_OS_INDEPENDENT
			+ "	headerValue = (char*) malloc(sizeof(char) * headerValueLength);" + CRLF_OS_INDEPENDENT
			+ "	strcpy(headerValue, header);" + CRLF_OS_INDEPENDENT
			+ "	if(lr_get_attrib_string(\"DynatraceLTN\")!=NULL){" + CRLF_OS_INDEPENDENT
			+ "		strcat(headerValue,\";LTN=\");" + CRLF_OS_INDEPENDENT
			+ "		strcat(headerValue,ltnString);" + CRLF_OS_INDEPENDENT
			+ "	}" + CRLF_OS_INDEPENDENT
			+ "	strcat(headerValue,\";VU=\");" + CRLF_OS_INDEPENDENT
			+ "	strcat(headerValue,vuserstring);" + CRLF_OS_INDEPENDENT
			+ "	strcat(headerValue,\";\");" + CRLF_OS_INDEPENDENT + CRLF_OS_INDEPENDENT
			+ "	web_add_header(\"X-dynaTrace-Test\", headerValue);" + CRLF_OS_INDEPENDENT
	...
	public static String JS_DYNATRACE_FUNCTION = "function addDynatraceHeaderTest(headerString) {"+CRLF_OS_INDEPENDENT
			+ "	result = lr.whoami();"+CRLF_OS_INDEPENDENT
			+ "	var vuserID = \"VU=\"+result[0];"+CRLF_OS_INDEPENDENT
			+ "	var header = headerString;"+CRLF_OS_INDEPENDENT
			+ "	var LTN = lr.getAttribString(\"DynatraceLTN\");"+CRLF_OS_INDEPENDENT
			+ "	if(LTN){"+CRLF_OS_INDEPENDENT
			+ "		header+= \"LTN=\"+LTN+\";\";"+CRLF_OS_INDEPENDENT
			+ "	}"+CRLF_OS_INDEPENDENT
			+ "	header += vuserID+';'" + CRLF_OS_INDEPENDENT
			+ "	web.addHeader(\"X-dynaTrace-Test\",header)"+CRLF_OS_INDEPENDENT
			+ "}";
	
	public static String[] JS_KEYWORDS = new String[] {
			"web.url","web.customRequest","web.submitData"
		};
	public static String[] JS_CLICK_AND_SCRIPT_KEYWORDS=new String[] {
			"web.image","web.link",
		};
}
